...
class Game {
    constructor(initialState){
        if (!initialState) {
            ...
            for(let i = 0; i < this.size; i++){
                const numberNull = [];
                for(let j = 0; j < this.size; j++)numberNull.push(0);
                this.board.push(numberNull);
            }
        } else if (initialState && Array.isArray(initialState)) this.board = initialState;
        this.score = 0;
        this.status = "idle";
    }
    prepareMove() {
        if (this.status !== "playing" && this.status !== "win") return null;
    ...
    handleMove(oldBoard) {
        ...
        if (hasChanged) this.allRandomTile();
        const has2048 = this.board.some((row)=>row.includes(2048));
        if (has2048 && this.status !== "win") this.status = "win";
        if (!this.canMove() && !has2048) this.status = "lose";
    }
    moveLeft() {
        const moveData = this.prepareMove();
        if (!moveData) return;
        const { oldBoard } = moveData;
        for(let i = 0; i < 4; i++)this.board[i] = this.processRow(this.board[i]);
    ...
    moveRight() {
        ...
        if (!moveData) return;
        const { oldBoard } = moveData;
        for(let i = 0; i < 4; i++)this.board[i] = this.processRow(this.board[i].reverse()).reverse();
        this.handleMove(oldBoard);
    }
    moveUp() {
        const moveData = this.prepareMove();
        ...
        for(let i = 0; i < 4; i++){
            transposed[i] = [];
            for(let j = 0; j < 4; j++)transposed[i][j] = this.board[j][i];
        }
        for(let i = 0; i < 4; i++)transposed[i] = this.processRow(transposed[i]);
        for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++)this.board[i][j] = transposed[j][i];
        this.handleMove(oldBoard);
    }
    moveDown() {
        const moveData = this.prepareMove();
        ...
        for(let i = 0; i < 4; i++)transposed[i] = this.processRow(transposed[i].reverse()).reverse();
        for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++)this.board[i][j] = transposed[j][i];
        this.handleMove(oldBoard);
    }
    canMove() {
    ...
    allRandomTile() {
        const emptyTile = [];
        for(let i = 0; i < this.size; i++){
            for(let j = 0; j < this.size; j++)if (this.board[i][j] === 0) emptyTile.push([
                i,
                j
            ]);
        }
        if (emptyTile.length === 0) return;
